...
def forward(
    ...
    if labels is not None:
        ...
        loss = loss_fct(logits[:, -120:-1].reshape(-1, self.decoder.config.vocab_size), labels[:, 1:].reshape(-1))
...
def collate_fn(batch):
    ...
    model_inputs['instruction_tokens'] = tokenization_fn(model_inputs['instruction_tokens'], max_target_length=50)
